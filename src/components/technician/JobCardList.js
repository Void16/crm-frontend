...
const JobCardList = ({ jobCards, onView, onEdit, onDelete, onUpdateStatus, loading }) => {
  ...
  const validateForm = () => {
    ...
    if (completionData.material_cost && parseFloat(completionData.material_cost) < 0) {
      errors.material_cost = 'Material cost cannot be negative';
    }
    
    if (completionData.customer_rating && (parseInt(completionData.customer_rating) < 1 || parseInt(completionData.customer_rating) > 5)) {
  ...
  const handleSubmitCompletion = async () => {
    ...
    try {
      const dataToSend = {
        ...
        material_cost: completionData.material_cost ? parseFloat(completionData.material_cost) : 0,
      };
      ...
      if (completionData.customer_rating) {
        dataToSend.customer_rating = parseInt(completionData.customer_rating);
      }
      
      console.log('Submitting completion data:', dataToSend);
      
      await onUpdateStatus(selectedJob.id, 'complete_job', dataToSend);
      ...
      <body>
        <div class="header">
          ...
          <div>
            ...
            <span class="status-badge status-${jobCard.status}">
              ${jobCard.status_display || jobCard.status.replace('_', ' ').toUpperCase()}
            </span>
          </div>
        </div>
        ...
        <div class="job-details">
          <div class="section">
            ...
            <div class="field-value">${jobCard.customer_email || 'N/A'}</div>
          </div>
          ...
          <div class="section">
            ...
            <div class="field-value">${jobCard.meter_type || 'N/A'}</div>
            
            <div class="field-label">Meter Reading:</div>
            <div class="field-value">${jobCard.meter_reading || 'N/A'}</div>
          </div>
        </div>
        ...
        <div class="section">
          ...
          <div class="field-value">${jobCard.job_type_display || jobCard.job_type}</div>
          
          <div class="field-label">Scheduled Date:</div>
          <div class="field-value">${formatDate(jobCard.scheduled_date)}</div>
          
          <div class="field-label">Due Date:</div>
          <div class="field-value">${formatDate(jobCard.due_date) || 'N/A'}</div>
          
          <div class="field-label">Priority:</div>
          <div class="field-value">${jobCard.priority_display || jobCard.priority || 'Medium'}</div>
          
          <div class="field-label">Job Description:</div>
          <div class="field-value">${jobCard.job_description || 'N/A'}</div>
        </div>
        ...
        <div class="section">
          ...
          <div class="field-value">${jobCard.hours_worked || '0'}</div>
        ...
        <div class="section">
          ...
          <div class="field-value">${'★'.repeat(jobCard.customer_rating)}${'☆'.repeat(5 - jobCard.customer_rating)} (${jobCard.customer_rating}/5)</div>
          ` : ''}
        </div>
        ` : ''}
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredJobCards.map((jobCard) => {
          const StatusIcon = statusIcons[jobCard.status] || AlertCircle;
            ...
            >
              ...
              <div className="p-5 border-b border-gray-100">
                <div className="flex justify-between items-start">
                  ...
                  <span className={`px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1.5 ${statusColors[jobCard.status]}`}>
                    <StatusIcon className="h-3.5 w-3.5" />
                    {jobCard.status_display || jobCard.status.replace('_', ' ')}
                  </span>
                </div>
              </div>
              ...
              <div className="p-5">
                <div className="space-y-4">
                  ...
                  <div className="flex items-start text-sm text-gray-700">
                    ...
                    <span className="truncate">{jobCard.customer_address.substring(0, 50)}...</span>
                  </div>
                  
                  <div className="flex items-start text-sm text-gray-700">
                    <Wrench className="h-4 w-4 mt-0.5 mr-3 text-gray-400 flex-shrink-0" />
                    <span>{jobCard.job_type_display || jobCard.job_type}</span>
                  </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    {jobCard.status === 'assigned' && (
                      <button
                        onClick={() => onUpdateStatus(jobCard.id, 'start_job')}
